  ...
  , m_timerPeriod(0.1)
{
  ...
  if (m_useRaycasting)
  {
    ...
    m_observationModel =
        boost::shared_ptr<ObservationModel>(new RaycastingModel(&m_privateNh, m_mapModel, &m_rngEngine));
  }
  else
  {
  ...
  m_globalLocSrv =
      m_nh.advertiseService("global_localization", &HumanoidLocalization::globalLocalizationCallback, this);
  ...
  m_pointCloudFilter =
      new tf::MessageFilter<sensor_msgs::PointCloud2>(*m_pointCloudSub, m_tfListener, m_odomFrameId, 100);
  ...
  m_initPoseFilter =
      new tf::MessageFilter<geometry_msgs::PoseWithCovarianceStamped>(*m_initPoseSub, m_tfListener, m_globalFrameId, 2);
  m_initPoseFilter->registerCallback(boost::bind(&HumanoidLocalization::initPoseCallback, this, _1));

  m_pauseIntegrationSub =
      m_privateNh.subscribe("pause_localization", 1, &HumanoidLocalization::pauseLocalizationCallback, this);
  m_pauseLocSrv =
      m_privateNh.advertiseService("pause_localization_srv", &HumanoidLocalization::pauseLocalizationSrvCallback, this);
  m_resumeLocSrv = m_privateNh.advertiseService("resume_localization_srv",
                                                &HumanoidLocalization::resumeLocalizationSrvCallback, this);
...
void HumanoidLocalization::reset()
{
  ...
  else
  {
    ...
    {  // useful for evaluation, when ground truth available:
      geometry_msgs::PoseStamped truePose;
      tf::Stamped<tf::Pose> truePoseTF;
      tf::Stamped<tf::Pose> ident(tf::Transform(tf::createIdentityQuaternion(), tf::Vector3(0, 0, 0)), ros::Time::now(),
                                  "torso_real");  // TODO: param

      ros::Time lookupTime = ros::Time::now();
      while (m_nh.ok() &&
             !m_tfListener.waitForTransform(m_globalFrameId, ident.frame_id_, lookupTime, ros::Duration(1.0)))
      {
        ROS_WARN("Waiting for transform %s --> %s for ground truth initialization failed, trying again...",
  ...
  ROS_INFO_STREAM("Initialization of " << m_numParticles << " particles took " << dt << "s (=" << dt / m_numParticles
                                       << "s/particle)");
#endif
}
...
void HumanoidLocalization::initZRP(double& z, double& roll, double& pitch)
{
  if (m_initPoseRealZRP)
  {
    ...
    double poseHeight;
    if (m_motionModel->getLastOdomPose(lastOdomPose) && lookupPoseHeight(lastOdomPose.stamp_, poseHeight))
    {
      z = poseHeight;
    }
    else
...
void HumanoidLocalization::laserCallback(const sensor_msgs::LaserScanConstPtr& msg)
{
  ...
  {  // no laser integration: propagate particles forward by full interval
...
bool HumanoidLocalization::localizeWithMeasurement(const PointCloud& pc_filtered, const std::vector<float>& ranges,
                                                   double max_range)
{
  ros::WallTime startTime = ros::WallTime::now();
#if PCL_VERSION_COMPARE(>=, 1, 7, 0)
  ...
  double dt = (ros::WallTime::now() - startTime).toSec();
  ROS_INFO_STREAM("Observations for " << m_numParticles << " particles took " << dt << "s (=" << dt / m_numParticles
                                      << "s/particle)");

  return true;
}

void HumanoidLocalization::prepareLaserPointCloud(const sensor_msgs::LaserScanConstPtr& laser, PointCloud& pc,
...
void HumanoidLocalization::filterGroundPlane(const PointCloud& pc, PointCloud& ground, PointCloud& nonground,
...
void HumanoidLocalization::prepareGeneralPointCloud(const sensor_msgs::PointCloud2::ConstPtr& msg, PointCloud& pc,
                                                    std::vector<float>& ranges) const
{
  pc.clear();
  ...
  if (m_groundFilterPointCloud)
  {
    ...
    ROS_INFO("PointCloudGroundFiltering done. Added %d non-ground points and %d ground points (from %zu). Cloud size "
             "is %zu",
...
void HumanoidLocalization::voxelGridSampling(const PointCloud& pc, pcl::PointCloud<int>& sampledIndices,
...
void HumanoidLocalization::pointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr& msg)
{
  ...
  if (!m_paused && (!m_receivedSensorData || isAboveHeadMotionThreshold || isAboveMotionThreshold(odomPose)))
  {
    ...
    double maxRange = 10.0;  // TODO #4: What is a maxRange for pointClouds? NaN? maxRange is expected to be a double
                             // and integrateMeasurement checks rangesSparse[i] > maxRange
    ROS_DEBUG("Updating Pose Estimate from a PointCloud with %zu points and %zu ranges", pc_filtered.size(),
              rangesSparse.size());
    sensor_integrated = localizeWithMeasurement(pc_filtered, rangesSparse, maxRange);
  }
  if (!sensor_integrated)
  {  // no observation necessary: propagate particles forward by full interval
    // relative odom transform to last odomPose
    tf::Transform odomTransform = m_motionModel->computeOdomTransform(odomPose);
    m_motionModel->applyOdomTransform(m_particles, odomTransform);
    constrainMotion(odomPose);
...
bool HumanoidLocalization::getImuMsg(const ros::Time& stamp, ros::Time& imuStamp, double& angleX, double& angleY) const
{
  ...
  if (closestOlderStamp < maxAge && closestNewerStamp < maxAge && closestOlderStamp + closestNewerStamp > 0.0)
  {
    ...
    imuStamp =
        ros::Time(weightOlder * closestOlder->header.stamp.toSec() + weightNewer * closestNewer->header.stamp.toSec());
...
void HumanoidLocalization::initPoseCallback(const geometry_msgs::PoseWithCovarianceStampedConstPtr& msg)
{
    ...
    if (m_initPoseRealZRP)
    {
      ...
      if (m_useIMU)
      {
        ...
        else
        {
          ...
          if (ok)
          {
            ROS_INFO("roll and pitch not set in initPoseCallback, use IMU values (roll = %f, pitch = %f) instead", roll,
                     pitch);
      ...
      if (useOdometry)
      {
        ...
        if (ok)
        {
          ...
          pose.setRotation(tf::createQuaternionFromRPY(roll, pitch, yaw));
          ROS_INFO("roll and pitch not set in initPoseCallback, use odometry values (roll = %f, pitch = %f) instead",
                   roll, pitch);
        }
        else
        {
          ROS_WARN("Could not determine current roll and pitch from odometry, falling back to init_pose_{roll,pitch} "
                   "parameters");
        }
      }
    }
  ...
  ROS_INFO("Pose reset around mean (%f %f %f)", pose.getOrigin().getX(), pose.getOrigin().getY(),
           pose.getOrigin().getZ());
...
void HumanoidLocalization::normalizeWeights()
{
  ...
  if (wmin > wmax)
  {
    ROS_ERROR_STREAM("Error in weights: min=" << wmin << ", max=" << wmax
                                              << ", 1st particle weight=" << m_particles[1].weight << std::endl);
  }
...
tf::Pose HumanoidLocalization::getMeanParticlePose() const
{
  ...
  meanPose.getBasis() =
      meanPose.getBasis().scaled(tf::Vector3(1.0 / m_numParticles, 1.0 / m_numParticles, 1.0 / m_numParticles));
